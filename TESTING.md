...
| `verify` | | compares legacy rows vs DB rows, DB hour rows vs `getHistory()` output, `PRAGMA integrity_check`, journal mode, migration markers | `checks.legacySubsetOfDb`, `checks.historyMatchesDb`, `checks.integrityOk`, `checks.walEnabled`, `checks.migrationMarked` (test markers, `test_*` meta keys), `checks.realMarkersIntact`, `checks.noCrashRows` |
